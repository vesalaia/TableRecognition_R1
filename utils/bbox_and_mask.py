...
def mergeRegions(opts, boxes, labels, masks, region1, region2, region3):
    ...
    if opts.DEBUG: 
        ...
        logging.info(f"Labels: {len(labels)} {labels}")
        logging.info(f"Boxes: {len(boxes)} {boxes}")
...
#                print(labels[i], labels[j], boxes[i], boxes[j], bb_iou(boxes[i],boxes[j]))
                try:
                    ...
                    logging.info(f"Exception: mergeBoxes failed")
                    
                if opts.DEBUG: logging.info(f"Box1: {boxes[i]} Box2:{boxes[j]} IoU: {bb_iou(boxes[i], boxes[j])}")
                labels[i] = region3
    ...
    for i in range(len(boxes)):
        for j in range(len(boxes)):
            ...
            if isboxAinsideboxB(boxes[i], boxes[j]):
                logging.info(f"Regions/Lines merged: {labels[i]} {labels[j]} {boxes[i]} {boxes[j]}")
                try:
