...
def createBbox(poly):
    l_xmax, l_ymax = poly.max(axis=0)
    l_xmin, l_ymin = poly.min(axis=0)
            ...
            try:
                ...
                if reg_poly.intersects(line_poly) and type(line_intersect) == shapely.geometry.polygon.Polygon:
                    ...
                    if (d_xmax - d_xmin) > opts.min_line_width and (d_ymax - d_ymin) > opts.min_line_heigth:
                        ...
                        if opts.debug: logging.info(f"Small line rejected: {d_xmin} {d_ymin} {d_xmax} {d_ymax}")
                        regline = {}
                else:
                    regline = {}
            except:
                logging.info(f"Invalid intersection++++++")
                logging.info(f"Region: [({r_xmin}, {r_ymin}), ({r_xmax}, {r_ymax})]")
                logging.info(f"Line: [({l_xmin}, {l_ymin}), ({l_xmax}, {l_ymax})]")
...
def map_lines_to_cell(opts, cell, lines):
    r_xmax, r_ymax = cell['polygon'].max(axis=0)
    r_xmin, r_ymin = cell['polygon'].min(axis=0)
    new_cell = {}
    new_cell['id'] = cell['id']
    new_cell['row'] = cell['row']
    new_cell['col'] = cell['col']
    new_cell['polygon'] = cell['polygon']
    cell_poly = make_valid(Polygon(cell['polygon']))
    ...
    for line in lines:
        ...
        if ((r_xmin - opts.line_boundary_margin) < l_xmin) and (l_xmin < (r_xmax + opts.line_boundary_margin)) and \
            ...
            line_poly = Polygon(line['Textline'])
            try:
                line_intersect = cell_poly.intersection(line_poly)
                if cell_poly.intersects(line_poly) and type(line_intersect) == shapely.geometry.polygon.Polygon:
                    detected_line = np.array(list(line_intersect.exterior.coords)).astype(int)
                    d_xmax, d_ymax = detected_line.max(axis=0)
                    d_xmin, d_ymin = detected_line.min(axis=0)
                    if (d_xmax - d_xmin) > opts.min_line_width and (d_ymax - d_ymin) > opts.min_line_heigth:
