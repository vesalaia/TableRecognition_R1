        ...
        for element in regions:
            for node in x.root.findall("".join([".//", x.base, element])):
                ...
                for line in node.findall("".join(["./", x.base, "TextLine"])):
                    ...
                    if self.DEBUG: logging.info(f"Line coords: {len(line_coords)}, {line_coords[0, 0]}, {line_coords[0, 1]}")
                    if self.DEBUG: logging.info(f"Baseline coords:, {len(baseline_coords)}, {baseline_coords[0, 0]}, {baseline_coords[0, 1]}")
...
#        for element in tables:
    ...
    def _debug__(self):
        ...
        # ensure that they are aligned
        for i in range(len(self.imgs)):
            logging.info(f"Image: {self.imgs[i]}")
            logging.info(f"XML: {self.xmls[i]}")
        logging.info(f"Length of imgs: {len(self.imgs)}")
        logging.info(f"Length of xmls: {len(self.xmls)}")
        logging.info(f"Classes: {self.classes}")

    def __imageinfo__(self, idx):
        logging.info(f"Image: {self.imgs[idx]}")
        # load images and masks
        xml_path = self.xmls[idx]
        x = xmlPAGE.pageData(xml_path)
        x.parse()
        ...
        for element in regions:
            for node in x.root.findall("".join([".//", x.base, element])):
                ...
                if rtype == None or rtype not in self.classes:
                    rtype = 'unknown'
                logging.info(f"Class: {self.classes[rtype]} Coords: {coords}")
