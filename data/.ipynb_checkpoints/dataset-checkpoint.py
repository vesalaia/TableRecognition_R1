...
def read_imageandxmlspairs(pairs):
    ...
    for item in pairs:
        ...
        for image in imagelist:
            if image.endswith(".jpg") or image.endswith(".TIF") or image.endswith(".jpeg"):
...
class OCRDatasetInstanceSeg(object):
    ...
    def __getitem__(self, idx):
        ...
        tlabels = []
        for item in obj_ids:
            tlabels.append(self.classes[self.reverse_Label[self._findLabel(item)]])
#        print(tlabels)
#        labels = torch.as_tensor(obj_ids, dtype=torch.int64)
        tlabels = torch.as_tensor(tlabels, dtype=torch.int64)
